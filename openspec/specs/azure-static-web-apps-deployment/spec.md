# Azure Static Web Apps Deployment Specification

## Purpose

Define how Ghosties is built, validated, and hosted on Azure Static Web Apps via Terraform and GitHub Actions, without storing secrets in the repository.
...
The project SHALL include Terraform configuration that creates Azure resource group `rg-ghosties-prod` in region `uksouth` and Static Web App `swa-ghosties-prod` in region `eastus2` with Free SKU, using committed non-secret variable values (no `terraform.tfvars.example` workflow).
...
- **THEN** it targets resource group `rg-ghosties-prod` in `uksouth` and Static Web App `swa-ghosties-prod` in `eastus2` with Free SKU
